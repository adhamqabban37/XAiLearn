...
export function CoursePreview({ initialCourse, onClear }: CoursePreviewProps) {
  ...
  console.log("[CoursePreview] Received course:", {
    hasInitialCourse: !!initialCourse,
    hasSessions: !!initialCourse?.sessions,
    sessionsLength: initialCourse?.sessions?.length,
    courseTitle: initialCourse?.course_title,
    fullCourse: initialCourse,
  });
  ...
  const handleStartLearning = () => {
    try {
      // Validate course has sessions and lessons
      if (!course?.sessions?.length) {
        console.error("[StartLearning] Course has no sessions");
        toast({
          variant: "destructive",
          title: "Invalid Course",
          description:
            "The course has no sessions. Please regenerate the course.",
        });
        return;
      }

      const hasLessons = course.sessions.some((s) => s.lessons?.length > 0);
      if (!hasLessons) {
        console.error("[StartLearning] Course has no lessons");
        toast({
          variant: "destructive",
          title: "Invalid Course",
          description:
            "The course has no lessons. Please regenerate the course.",
        });
        return;
      }
      ...
      console.log(
        `[StartLearning] Starting session with duration: ${dbgDuration}m`
      );
      console.log(`[StartLearning] Course sessions:`, course.sessions.length);
      console.log(
        `[StartLearning] Total lessons:`,
        course.sessions.reduce((acc, s) => acc + s.lessons.length, 0)
      );
      ...
      if (newSession) {
        ...
        toast({
          ...
          title: "Session Error",
          description:
            "Could not start a new session. The course might be fully completed or have no lessons.",
        });
      }
    } catch (err: any) {
    ...
    <div className="w-full max-w-4xl mx-auto space-y-4 sm:space-y-6">
      <Card>
        <CardHeader className="p-4 sm:p-6">
          <CardTitle className="text-2xl sm:text-3xl font-headline break-words">
            {course.course_title}
          </CardTitle>
        ...
        <CardContent className="space-y-4 sm:space-y-6 p-4 sm:p-6 pt-0">
          <Card className="bg-muted/30">
            <CardHeader className="p-4 sm:p-6">
              <CardTitle className="flex items-center gap-2 text-lg sm:text-xl">
                <Sparkles className="text-primary w-5 h-5 sm:w-6 sm:h-6 shrink-0" />{" "}
                Analysis Report
              </CardTitle>
              <CardDescription className="text-sm">
                The AI has analyzed your document. Here's what it found.
              </CardDescription>
            </CardHeader>
          ...
          <div className="space-y-2">
            ...
            <Accordion
              type="single"
              collapsible
              className="w-full border rounded-md px-3 sm:px-4"
              defaultValue="session-0"
            >
              {course.sessions.map((session, sIndex) => (
                <AccordionItem
                  value={`session-${sIndex}`}
                  key={session.id}
                  className={
                    sIndex === course.sessions.length - 1 ? "border-b-0" : ""
                  }
                >
                  ...
                  <AccordionContent>
                    <ul className="space-y-2 sm:space-y-3 pl-4 sm:pl-6 border-l-2 ml-2">
                      {session.lessons.map((lesson) => (
                        <li
                          key={lesson.id}
                          className="text-muted-foreground pl-2 text-sm sm:text-base"
                        >
                          ...
                          {lesson.key_points &&
                            lesson.key_points.length > 0 && (
                              <ul className="text-xs sm:text-sm italic mt-1 list-disc pl-3 sm:pl-4 leading-relaxed">
                                {lesson.key_points.map((point, pIndex) => (
        ...
        <CardFooter className="flex flex-col items-stretch gap-4 bg-muted/50 p-4 sm:p-6 rounded-b-lg">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full">
            ...
            <Button
              ...
              className="text-base sm:text-lg touch-target w-full sm:flex-1"
            >
              Start Learning
            </Button>
          </div>
